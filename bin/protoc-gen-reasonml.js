...
const preBoilerPlate = `
  /* opaque type needed to add services to a grpc server instance */
  type protoHandle;
  [@bs.module "grpc"]
  external grpcLoadProto : string => protoHandle = "load";
...
require('read-all-stream')(process.stdin, {encoding:null}).then(buf => {
  ...
  const emitModule = module => {
    ...
    if ('fileName' in module) {
       ...
       * TODO escape path
       */
      code += `let myProtoHandle = grpcLoadProto("${module.fileName}");`
    }
    ...
    if ('rpcs' in module) {
      ...
      code += `};
        [@bs.send] external addService : (server, grpcServiceClient, t) => unit = "";
        let addService = (server, t) =>
          addService(server, myServiceClient, t);
      `
    }
  ...
  function preLoader(rootModule) {
    let code = '%bs.raw {|\nconst loadedProtos = {}\n';
    for (moduleName in rootModule.modules) {
      const module = rootModule.modules[moduleName]
      const moduleNameString = JSON.stringify(module.moduleName)
      const fileNameString = JSON.stringify(module.fileName)
      code += `loadedProtos[${moduleNameString}] = require('grpc').load(${fileNameString})\n`
    }
    code += '|};\n'
    return code
  }
  ...
  const emission = FILE_MAGIC
    + preLoader(rootModule) /* javascript code to load each .proto file */
    + preBoilerPlate
    + emitModule(rootModule)
    + boilerPlate
