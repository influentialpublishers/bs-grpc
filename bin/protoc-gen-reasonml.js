 ...
 * size? */
const FILE_MAGIC_NUM_BYTES = (new Buffer(FILE_MAGIC, 'utf8')).length;
...
const moduleAndAllAncestors = moduleName =>
  moduleName.split('.').map((_, i, a) => a.slice(i).join('.'))
// TODO ramda?
const eachProp = (o,f) => {
  for (let k in o)
    f(k, o[k])
...
/*const determineModuleStorage = (packageName, outDir) => {
  const packageNameComponents = packageName.split('.').map(upper1)
  for (let i=packageNameComponents.length-2; i>=0; i--) {
    = packageNameComponents
      .slice(i, packageNameComponents.length-2)
      .join('_')
      + '.re'
    const dirPath = Path.join.apply(null, dirPathComponents)
